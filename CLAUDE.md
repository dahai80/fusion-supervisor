...
- `src/main.rs` — CLI (`clap`) entry, daemon tick loop, RPC client wiring. Issue #10: `Commands::Backup` → `cli_call(cfg, "backup")`; tick loop 在 `tick_all` 后调 `c.check_backup_schedule(now)` (到期触发 run_backup, fail-closed 已告警)。Issue #20: `run_daemon` 构造 `oneshot::channel<()>`, tx 传入 `serve_with_shutdown`, main loop `tokio::select!` 监听 `ctrl_c()` 或 shutdown rx, 任一触发 `tick_handle.abort()` + 优雅返回。`Commands::Shutdown` → `cli_call(cfg, "shutdown")`
