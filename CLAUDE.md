...
- `src/config.rs` — `Config` struct + `~`-expansion (tilde must be expanded, never left literal — see `test_load_expands_tilde_in_db_path`). Issue #9 fields: `drain_grace_sec` (serde default 30 — drain 宽限期, 到期强制 Stopped), `rollout_prewarm_timeout_sec` (serde default 60 — rollout prewarm 超时, 未达 Healthy 则 Stopped + 告警)
...
- `src/rpc/` — `server.rs` (dispatch), `client.rs`, `schema.rs` (envelope + `Method` enum). Issue #9: `Method::Drain` (params.service 单服务或无=drain_all) + `Method::Rollout` (params.service 必填)。server.rs Drain 用 SystemTime now_ts; Rollout 先读 `cfg.drain_grace_sec`/`cfg.rollout_prewarm_timeout_sec` 到局部再 `c.rollout(...).await` (避 E0502 读写冲突)
