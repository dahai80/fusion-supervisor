...
- `src/main.rs` — CLI (`clap`) entry, daemon tick loop, RPC client wiring. Issue #10: `Commands::Backup` → `cli_call(cfg, "backup")`; tick loop 在 `tick_all` 后调 `c.check_backup_schedule(now)` (到期触发 run_backup, fail-closed 已告警)
