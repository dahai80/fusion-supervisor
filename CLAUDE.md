...
fusion-sv drain [svc]     # drain traffic from one service (or all if no svc) — issue #9
fusion-sv rollout <svc>   # zero-downtime rollout: drain→stop→start→prewarm — issue #9
fusion-sv backup          # one-shot backup: Postgres (pg_dumpall→gzip) + Qdrant → FUSION_BACKUP_TARGET — issue #10
...
- `src/main.rs` — CLI (`clap`) entry, daemon tick loop, RPC client wiring. Issue #10: `Commands::Backup` → `cli_call(cfg, "backup")`; tick loop 在 `tick_all` 后调 `c.check_backup_schedule(now)` (到期触发 run_backup, fail-closed 已告警)。Issue #20: `run_daemon` 构造 `oneshot::channel<()>`, tx 传入 `serve_with_shutdown`, main loop `tokio::select!` 监听 `ctrl_c()` 或 shutdown rx, 任一触发 `tick_handle.abort()` + 优雅返回。`Commands::Shutdown` → `cli_call(cfg, "shutdown")`。Issue #22: `run_daemon` 第三个 `tokio::spawn` (gated on `cfg.metrics_addr.is_some()`) 跑 `serve_metrics(core.clone(), addr)` — hand-rolled HTTP `/metrics` server (`tokio::net::TcpListener` + `tokio::io`, 无 hyper/axum 新依赖), accept 循环 → 每连接 `handle_metrics_conn`: 读请求行, 仅 `GET /metrics` → 200 text/plain (content-type `text/plain; version=0.0.4`), 其他 → 404。锁纪律: handler `lock().await` → `metrics::render(&c)` → 立即释放 (短持锁同 RPC, 绝不跨 await)。daemon 退出时 `metrics_handle.abort()` (同 tick_handle)。`serve_metrics`/`handle_metrics_conn` 为进程内 free fn (非 lib 导出, 测试在 `tests/test_metrics.rs` 内复制同构 HTTP server 验证端到端)
- `src/lib.rs` — module re-exports
- `src/config.rs` — `Config` struct + `~`-expansion (tilde must be expanded, never left literal — see `test_load_expands_tilde_in_db_path`). Issue #9 fields: `drain_grace_sec` (serde default 30 — drain 宽限期, 到期强制 Stopped), `rollout_prewarm_timeout_sec` (serde default 60 — rollout prewarm 超时, 未达 Healthy 则 Stopped + 告警). Issue #10 fields: `backup_schedule_sec` (serde default 86400 — daemon tick 循环检查到期触发 backup), `backup_retention` (serde default 7 — 新备份后裁剪超出 N 份的旧备份). Issue #22 field: `metrics_addr: Option<String>` (serde default `default_metrics_addr` → `Some("127.0.0.1:9100")` — localhost-only 绑定是安全边界, 无 auth); `load()` 中空字符串 trim 后归一化为 `None` (禁用端点)
- `src/metrics.rs` — issue #22: Prometheus /metrics 纯逻辑渲染。`render(&SupervisorCore) -> String` 生成 text exposition format v0.0.4, 无 HTTP/无锁/无 DB (调用方 handler 负责 lock 后传引用)。指标: `fusion_sv_up 1` (端点响应即活), `fusion_sv_services_total{plane}` (native=runners.len, compose=若 enabled 则 status_all().filter plane=="compose" count), `fusion_sv_service_healthy{service,port,plane}` (is_healthy: Healthy→1 else 0), `fusion_sv_service_state{...state}` (1 for 当前态, `state_label` 映射 7 个 State 变体全小写), `fusion_sv_crash_count{service}` (`r.policy.crash_count()`, native only). compose 容器无 crash_count (Docker restart policy own)。4 unit tests (render 输出断言: daemon up+counts / healthy+state+crash gauges / Failed healthy=0 / 7 State 变体全覆盖)
...
- `src/alert.rs` — `Alerter`: `ServiceDown` deduped 5min/service; other kinds always emit. `AlertKind::ContainerStateChange` + `ContainerCrashLoop` (issue #6, compose-plane) always emit; `ContainerCrashLoop` logs at ERROR. `AlertKind::ServiceDrained` (issue #9, drain 完成) + `RolloutPrewarmFailed` (issue #9, rollout prewarm 超时未达 Healthy) — normal/escalation events. `AlertKind::BackupFailed` (issue #10, 备份 fail-closed: target 未设/不可写/docker exec 失败). Sink = tracing WARN + optional webhook (3-retry, fails back to log only)
- `src/backup.rs` — issue #10: NAS 备份原语 `BackupRunner`。injectable `ExecFn = Arc<dyn Fn(&str, &[&str]) -> Result<Vec<u8>>>` (生产=`real_exec` shell `docker compose --env-file ... -f sidecar -f business exec <svc> <args>`, 测试=fake 返回固定 bytes)。`run(now_ts, &HistoryStore, &mut Alerter)` fail-closed: `FUSION_BACKUP_TARGET` (env 优先, 再 `.env` 文件) 未设 → `bail!` + `BackupFailed` 告警; 否则 `pg_dump` (`pg_dumpall -U fusion` → gzip via flate2 `GzEncoder`) + `qdrant_snapshot` (`tar cf - -C /qdrant/storage .` → `storage.tar`) + `prune_old` (按文件名 date 词序降序, 保留 `retention` 份, 删最旧 + 对应 `fusion-qdrant-{date}` 目录) + `record_outcome` (写 history.db `kind=Backup` event)。store/alerter 借用传入 (Connection 不可 Clone, 故 split-borrow)。free fns: `resolve_target()` (env→.env), `backup_due(now, last, schedule)` (纯函数: last==0 恒到期, 否则 `now - last >= schedule`), `date_stamp()` (env `FUSION_BACKUP_DATE` 覆盖便于测试, 否则 `d{day}`)
...
- `src/env_map.rs` — issue #8: compose enabled 时, 依赖 sidecar 的 native 服务需显式注入连接 env。`required_vars(svc)` maps fusion-identity→[DATABASE_URL], fusion-rag→[QDRANT_URL], fusion-cowork/fusion-model-hub→[DATABASE_URL,REDIS_URL]; `needs_injection(svc)` is the gate. `native_env_for(svc, env_file) -> Result<HashMap<String,String>>` resolves values (process env precedence over `.env`): DATABASE_URL=`postgresql://{FUSION_PG_USER}:{FUSION_PG_PASSWORD}@localhost:5432/{db}` (fail-closed — missing FUSION_PG_PASSWORD → Err), REDIS_URL=`redis://localhost:6379/1`, QDRANT_URL=`http://localhost:6333`. Non-dependent services → empty map (no injection). `pg_db_for` derives per-service PG db name from FUSION_*_DB env (fallback: service name minus `fusion-` prefix)
- `src/supervisor/mod.rs` — `SupervisorCore`: `up_all`/`down_all` (layered, per-layer concurrency cap via `Semaphore`), `tick_all`, `poll_interval` (switches to `poll_unhealthy_sec` when any runner is down). Production `new()` uses real `bash start.sh` + real probe; `new_with_fns()` injects fakes for tests. `ArcStartFn` signature is `Fn(&str /*repo_dir*/, &str /*cmd*/, &[(String, String)] /*env_overrides*/) -> Result<()>` — the env_overrides param (issue #8) carries injected sidecar connection vars. `up_native` resolves env_overrides via `env_map::native_env_for` when `compose.enabled && needs_injection`; **fail-closed**: env resolution Err → refuses to start that service, sets runner.state=Failed, logs ERROR (next tick handles alerting — up_native stays free of async .await alert). `down_native` passes `&[]`. Crash recovery (`runner.invoke_start`) + manual restart (`rpc/server.rs`) use the runner's internal `StartFn` (unchanged 1-arg `Fn(&str)`, reuses start.sh's own .env reading) — documented scope per issue #8. Issue #9 drain/rollout primitives: `drain(service, now_ts)` (单服务 begin_drain), `drain_all(now_ts)` (全部, 任一 Err bail), `cancel_drain(service)` (Draining→Healthy), `rollout(service, now_ts, grace, prewarm_timeout)` async (drain→mark Stopped→invoke_start→Starting→prewarm loop 用 `runner.probe_once()` 到 deadline=now_ts+prewarm_timeout; 达 Healthy→return Ok, 超时→Stopped + `RolloutPrewarmFailed` 告警 + bail). **Coordination boundary**: supervisor owns local lifecycle; fusion-gateway owns traffic cut-over; fusion-multi-node owns cluster drain. Issue #10: `run_backup(now_ts)` 构造 `BackupRunner::new(&cfg)` 调 `run(now_ts, &store, &mut alerter)`, 成功则记 `last_backup_ts`; `check_backup_schedule(now_ts)` 用 `backup_due` 判定到期触发 `run_backup` (失败也更新 last_backup_ts 避免每 tick 重试, fail-closed 已告警)。`last_backup_ts: u64` 字段 (0=从未备份 → 首次 tick 即到期)
- `src/supervisor/runner.rs` — `ServiceRunner` + state machine `Stopped→Starting→Healthy→(Unhealthy)→Restarting→Healthy|Failed`, plus issue #9 drain path: `Healthy→Draining→Stopped` (drain 完成) or `Healthy→Draining→Healthy` (drain 取消). `State::Draining` (探活仍跑但 tick 不重启; grace 到期或探活不可达 → `Stopped` + `ServiceDrained` 告警). `drain_grace_until: Option<u64>` field. `begin_drain(grace_sec, now_ts)` (仅 Healthy/Starting/Unhealthy 可 drain), `cancel_drain()` (仅 Draining 可取消 → Healthy), `probe_once()` (单次探活, rollout prewarm 用 — 复用注入的 probe_fn). `handle_unhealthy` 的 `State::Draining` arm 是防御性兜底 (tick 提前返回, 理论不可达), 满足穷尽匹配
...
- `src/rpc/` — `server.rs` (dispatch), `client.rs`, `schema.rs` (envelope + `Method` enum). Issue #9: `Method::Drain` (params.service 单服务或无=drain_all) + `Method::Rollout` (params.service 必填)。server.rs Drain 用 SystemTime now_ts; Rollout 先读 `cfg.drain_grace_sec`/`cfg.rollout_prewarm_timeout_sec` 到局部再 `c.rollout(...).await` (避 E0502 读写冲突)。Issue #10: `Method::Backup` (无 params) → `c.run_backup(now_ts)`, 成功 `json!("ok")`, 失败 `-32000` (fail-closed 已告警)。Issue #20: `serve_with_shutdown(core, sock, token, shutdown_tx: Option<oneshot::Sender>)` — Shutdown arm 取出 tx 发送信号 (take 后置 None → 幂等, 重复调用返回 `-32000 shutdown channel unavailable`), 回复 `json!("ok")`。tx 经 `Arc<Mutex<Option<Sender>>>` 共享到每个 `handle_conn` task。`Method::Top` 是客户端命令 (cli_top 循环 cli_status), server arm 返回 `-32601` 带说明 "run `fusion-sv top` directly" (enum 保留, `test_parse_method` 仍断言 SHUTDOWN/TOP 解析)。`serve()` (3 参数) 为向后兼容保留, 内部转调 `serve_with_shutdown(.., None)`
...
- **up_all order**: `up_sidecar()` (traefik/redis/postgres/qdrant, `--wait`) → `up_native()` (layer-ordered, concurrency cap, probe-skip-healthy, **dep-gate**) → `up_business()` (model-hub/cowork, `--wait`, depends on sidecar + native gateway).
- **dep-gate (issue #5)**: `up_native` explicitly checks that the sidecar containers a native service depends on are `Healthy` before starting it — does NOT rely on layer ordering. The dependency map lives in `src/dep_map.rs` (OnceLock `HashMap`, same pattern as `probe_map.rs`): `fusion-identity→postgres`, `fusion-rag→qdrant`, `fusion-cowork→postgres+redis`, `fusion-model-hub→postgres+redis`. `dep_gate_ok` returns true when compose is disabled (single-node dev preserved) or the service has no mapped deps; on an unhealthy/missing dep it skips startup (marks runner `Stopped`, NOT `Failed` — a dependency problem is not a crash) and emits a `ServiceDown` alert. `ps()` failure degrades to pass-open (avoids false blocking on a flaky compose query).
